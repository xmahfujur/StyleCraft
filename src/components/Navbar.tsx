...
export default function Navbar() {
  ...
  const { wishlist, removeFromWishlist, isWishlistOpen, setIsWishlistOpen, isSearchOpen, setIsSearchOpen } = useWishlist();
  ...
  useEffect(() => {
    if (isSearchOpen || isCartOpen || isWishlistOpen || isMenuOpen) {
  ...
  }, [isSearchOpen, isCartOpen, isWishlistOpen, isMenuOpen]);
            ...
            <div className="flex items-center gap-4 px-6 py-4 border-b border-black/5 bg-white shrink-0">
              ...
              <form onSubmit={handleSearch} className="flex-1 relative">
                <input 
                  ...
                  className="w-full bg-black/5 border-none rounded-full py-2.5 pl-10 pr-4 text-sm font-medium focus:ring-0 outline-none text-black placeholder:text-black/30"
                />
              ...
              <button 
                ...
                className="text-sm font-black tracking-widest text-black uppercase active:scale-95 transition-transform"
              >
                SEARCH
              </button>
            </div>
              ...
              <div className="max-w-3xl mx-auto space-y-10">
                ...
                <section>
                  <div className="flex items-center gap-2 mb-4 text-black/40">
                    <span className="text-[10px] font-black tracking-[0.3em] uppercase">Popular Searches</span>
                    <div className="h-px bg-black/5 flex-1" />
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {['Premium Shirts', 'New Arrivals', 'Polo Shirts', 'Cotton Pants', 'Trending', 'Best Sellers'].map((term) => (
                      <button
                        key={term}
                        onClick={() => {
                          setSearchQuery(term);
                        ...
                        className="bg-black/5 hover:bg-black hover:text-white px-4 py-2 rounded-full text-[10px] font-bold tracking-widest uppercase transition-all duration-300"
                      >
                        {term}
                      </button>
                    ))}
                ...
                <section>
                  <div className="flex items-center gap-2 mb-4 text-black/40">
                    <span className="text-[10px] font-black tracking-[0.3em] uppercase">Browse Collections</span>
                    <div className="h-px bg-black/5 flex-1" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    {collections.map((col) => (
                      <button
                        ...
                        className="group relative h-24 overflow-hidden rounded-xl bg-black/5 text-left border border-black/5 hover:border-black/20 transition-all"
                      >
                        <div className="absolute inset-0 bg-gradient-to-r from-white via-white/80 to-transparent z-10 p-4 flex items-center">
                          <span className="text-[10px] font-black tracking-widest uppercase text-black group-hover:translate-x-1 transition-transform">{col.name}</span>
                        </div>
                        {col.bannerImage && (
                          <img 
                            src={col.bannerImage} 
                            alt={col.name} 
                            className="absolute right-0 top-0 h-full w-1/2 object-cover opacity-50 group-hover:scale-110 group-hover:opacity-100 transition-all duration-500"
                          />
                        )}
                      </button>
                    ))}
                ...
                <section className="bg-black/5 p-6 rounded-2xl">
                  <div className="grid grid-cols-2 gap-6">
                    <div>
                      <h4 className="text-[10px] font-black tracking-widest text-black uppercase mb-3">Customer Support</h4>
                      <div className="space-y-2">
                        <p className="text-[9px] font-bold text-black/40 uppercase tracking-widest">Order Tracking</p>
                        <p className="text-[9px] font-bold text-black/40 uppercase tracking-widest">Return Policy</p>
                        <p className="text-[9px] font-bold text-black/40 uppercase tracking-widest">Size Guide</p>
                      </div>
                    </div>
                    <div>
                      <h4 className="text-[10px] font-black tracking-widest text-black uppercase mb-3">Company</h4>
                      <div className="space-y-2">
                        <p className="text-[9px] font-bold text-black/40 uppercase tracking-widest">About Us</p>
                        <p className="text-[9px] font-bold text-black/40 uppercase tracking-widest">Store Locator</p>
                        <p className="text-[9px] font-bold text-black/40 uppercase tracking-widest">Privacy Policy</p>
                      </div>
                    </div>
                  </div>
                </section>
        ...
        {isWishlistOpen && (
          <>
            ...
            <motion.div 
              ...
              className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-white border-l border-black/10 z-[2000] flex flex-col shadow-2xl h-screen"
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 min-h-0 custom-scrollbar bg-white pb-32 md:pb-6">
                {wishlist.length === 0 ? (
        ...
        {isCartOpen && (
          <>
            ...
            >
                    ...
                    <div className="space-y-6">
                      {cart.map((item) => (
                        <div key={`${item.id}-${item.size}`} className="flex space-x-4 bg-black/[0.02] p-3 border border-black/5">
                          <img src={item.image || undefined} className="w-16 h-20 object-cover flex-shrink-0 bg-white" referrerPolicy="no-referrer" />
              ...
              {/* Footer - Fixed Bottom */}
              {cart.length > 0 && (
                <div className="p-6 border-t border-black/5 bg-white space-y-4 z-10 shadow-[0_-10px_20px_rgba(0,0,0,0.05)] pb-32 md:pb-6 safe-bottom">
                  <div className="space-y-2">
