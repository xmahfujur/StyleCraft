...
import { ShoppingBag, User, LogOut, ShieldCheck, Menu, X, Sun, Moon, Trash2, Plus, Minus, ArrowRight, Heart, ChevronDown } from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[60] bg-luxury-black/80 backdrop-blur-md border-b border-gold/10" onMouseLeave={() => setActiveDropdown(null)}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <Link to="/" className="flex items-center space-x-3">
            ...
            <span className="text-2xl font-serif font-bold tracking-widest text-gold hidden lg:block uppercase">STYLECRAFT</span>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            >
              <button className={`text-[10px] tracking-[0.3em] font-bold uppercase transition flex items-center gap-1 ${location.pathname.includes('/collection') ? 'text-gold' : 'text-white/70 hover:text-gold'}`}>
                COLLECTIONS
                <ChevronDown size={10} />
              </button>
              <AnimatePresence>
                {activeDropdown === 'collections' && (
                  <motion.div 
                    ...
                    exit={{ opacity: 0, y: 10 }}
                    className="absolute top-full left-0 w-64 bg-luxury-black border border-gold/10 p-4 space-y-2 shadow-2xl"
                  >
                    {collections.map(col => (
                      <Link key={col.id} to={`/collection/${col.slug}`} className="block text-[9px] tracking-widest text-white/50 hover:text-gold uppercase py-2 border-b border-white/5 last:border-0 transition" onClick={() => setActiveDropdown(null)}>
                        {col.name}
                      </Link>
                    ))}
                    {collections.length === 0 && <p className="text-[9px] text-white/20 italic">No collections</p>}
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            ...
            >
              <button className={`text-[10px] tracking-[0.3em] font-bold uppercase transition flex items-center gap-1 ${location.pathname.includes('/category') ? 'text-gold' : 'text-white/70 hover:text-gold'}`}>
                CATEGORIES
                <ChevronDown size={10} />
              </button>
              <AnimatePresence>
                {activeDropdown === 'categories' && (
                  ...
                  >
                    {categories.map(cat => (
                      <Link key={cat.id} to={`/category/${cat.slug}`} className="block text-[9px] tracking-widest text-white/50 hover:text-gold uppercase py-2 border-b border-white/5 last:border-0 transition" onClick={() => setActiveDropdown(null)}>
                        {cat.name}
                      </Link>
                    ))}
                    {categories.length === 0 && <p className="text-[9px] text-white/20 italic">No categories</p>}
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            ...
            {isAdmin && (
              <Link to="/admin" className="flex items-center space-x-1 text-[10px] tracking-[0.3em] font-bold text-gold hover:brightness-125 transition uppercase">
                <ShieldCheck size={14} />
                <span>ADMIN</span>
              </Link>
            )}
      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            <Link to="/" onClick={() => setIsMenuOpen(false)} className="block text-[10px] tracking-[0.3em] font-bold text-center uppercase">HOME</Link>
            
            {collections.length > 0 && (
              <div className="space-y-4">
                <p className="text-[8px] tracking-[0.4em] text-white/20 text-center uppercase">Collections</p>
                {collections.map(col => (
                  <Link key={col.id} to={`/collection/${col.slug}`} onClick={() => setIsMenuOpen(false)} className="block text-[10px] tracking-widest text-center uppercase text-white/50">{col.name}</Link>
                ))}
              </div>
            )}
            ...
            {categories.length > 0 && (
              <div className="space-y-4">
                <p className="text-[8px] tracking-[0.4em] text-white/20 text-center uppercase">Categories</p>
                {categories.map(cat => (
                  <Link key={cat.id} to={`/category/${cat.slug}`} onClick={() => setIsMenuOpen(false)} className="block text-[10px] tracking-widest text-center uppercase text-white/50">{cat.name}</Link>
                ))}
              </div>
            )}
                ...
                {cart.length === 0 ? (
                  ...
                  <div className="space-y-8 pb-4">
                    <div className="space-y-6">
                      {cart.map((item) => (
                        <div key={`${item.id}-${item.size}`} className="flex space-x-4 bg-white/5 p-3 border border-white/5">
                          <img src={item.image || null} className="w-16 h-20 object-cover flex-shrink-0" referrerPolicy="no-referrer" />
                          <div className="flex-1 min-w-0">
                            <div className="flex justify-between">
                              <h3 className="text-xs font-serif truncate pr-2 uppercase tracking-wider">{item.name}</h3>
                              <button onClick={() => removeFromCart(item.id, item.size)} className="text-white/20 hover:text-red-400 p-1 flex-shrink-0">
                                <Trash2 size={12} />
                              </button>
                            </div>
                            <p className="text-[9px] text-gold tracking-[0.2em] uppercase mt-0.5">SIZE: {item.size}</p>
