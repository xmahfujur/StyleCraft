        ...
        <div className="lg:col-span-1">
          <div className="bg-white/5 border border-gold/10 p-8 sticky-checkout">
            ...
            <div className="space-y-4 mb-8">
              ...
              {totalItems <= 2 && (
                <p className="text-[10px] text-gold/60 italic tracking-wider">
                  Add {3 - totalItems} more {3 - totalItems === 1 ? 'item' : 'items'} for FREE delivery
                </p>
              )}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        <button
                          ...
                          className={`py-2 px-1 text-[9px] tracking-widest uppercase border transition flex flex-col items-center justify-center gap-1 ${location === loc.label ? 'bg-gold text-luxury-black border-gold' : 'border-white/10 text-white/40'}`}
                        >
                          <span className="font-bold">{loc.label}</span>
                          <span className="opacity-60">{totalItems > 2 ? 'Free' : `${loc.price} Taka`}</span>
                        </button>
                      ))}
                    </div>
                  </div>
              ...
              <div className="space-y-6 mb-8">
                <div className="bg-white/5 p-4 border border-white/5">
                  ...
                  <div className="text-sm space-y-1 text-white/80">
                    <p className="font-bold text-white">{formData.fullName}</p>
                    <p>{formData.phone}</p>
                    <p>{[formData.addressLine, formData.area].filter(Boolean).join(', ')}</p>
                  </div>
                </div>
