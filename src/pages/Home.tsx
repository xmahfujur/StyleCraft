...
import { ShoppingBag, ChevronRight, Plus, Search, Filter, X, Truck, Heart } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <div className="pt-20">
      ...
      >
        <div className="flex flex-col md:flex-row items-center gap-4 text-gold group cursor-default hover:scale-[1.02] transition-transform duration-500">
          <motion.div
            animate={{ 
          ...
          <div className="text-center">
            <h3 className="text-lg md:text-2xl font-serif tracking-[0.1em] leading-relaxed">
              Buy 3 items at once and enjoy{" "}
              <span className="relative inline-block font-bold italic group-hover:text-white transition-colors duration-500">
                FREE delivery
                <motion.span 
                  animate={{ 
                    opacity: [0.3, 1, 0.3],
                  ...
                  className="absolute -bottom-1 left-0 w-full h-[1px] bg-gold shadow-[0_0_8px_#D4AF37]"
                />
              </span>{" "}
        ...
        <motion.div 
          ...
          className="h-px bg-gold/20 mt-8" 
        />
      </motion.div>
            ...
            <div className="relative group">
              ...
              {searchQuery && (
                <>
                  ...
                  <div className="absolute top-full left-0 right-0 mt-2 bg-luxury-black border border-gold/20 shadow-2xl z-50 max-h-[400px] overflow-y-auto custom-scrollbar">
                      ...
                      <div className="p-2">
                        {filteredProducts.slice(0, 5).map(p => (
                          <Link 
                            key={p.id} 
                            to={`/product/${p.id}`}
                            className="flex items-center space-x-4 p-3 hover:bg-white/5 transition group"
                          >
                            <div className="w-12 h-16 bg-white/5 flex items-center justify-center overflow-hidden">
                              <img src={p.images?.[0] || p.image || undefined} className="max-w-full max-h-full object-contain" referrerPolicy="no-referrer" />
                            </div>
                            <div>
                              <p className="text-xs font-serif group-hover:text-gold transition uppercase tracking-wider">{p.name}</p>
                              <p className="text-[10px] text-white/40 mt-1 uppercase tracking-widest">৳{p.discountedPrice || p.price}</p>
                            </div>
                          </Link>
                        ))}
                        {filteredProducts.length > 5 && (
                          <div className="p-3 text-center border-t border-white/5">
                            <p className="text-[10px] text-gold tracking-widest uppercase">+{filteredProducts.length - 5} more results</p>
                          </div>
                        )}
                      </div>
                    ) : (
                      <div className="p-6 text-center">
                        <p className="text-[10px] text-white/40 tracking-widest uppercase">No results found</p>
                      </div>
                    )}
                  </div>
                </>
                ...
                <Link to={`/product/${product.id}`} className="flex flex-col h-full">
                  <div className="relative aspect-[3/4] mb-4 bg-white/5 flex items-center justify-center overflow-hidden">
                    <img 
                      ...
                      alt={product.name} 
                      className={`max-w-full max-h-full object-contain transition-all duration-700 scale-105 group-hover:scale-100 block ${product.images && product.images.length > 1 ? 'group-hover:opacity-0' : ''}`}
                      referrerPolicy="no-referrer"
                    />
                    {product.images && product.images.length > 1 && (
                      <img 
                        ...
                        className="absolute inset-0 w-full h-full object-contain transition-all duration-700 opacity-0 group-hover:opacity-100 scale-100 group-hover:scale-105"
                        referrerPolicy="no-referrer"
                      />
                    )}
