  ...
  const handleSocialLogin = async () => {
    ...
    try {
      ...
      if (auth.currentUser?.isAnonymous) {
        try {
          // Attempt to link anonymous account to social account
          res = await linkWithPopup(auth.currentUser, provider);
        } catch (linkError: any) {
          // If the Google account is already in use by another user, fall back to sign-in
      ...
      } else if (userDoc.data()?.role === 'guest') {
        // Upgrade guest profile to user profile
      ...
      if (error.code === 'auth/unauthorized-domain') {
        toast.error('This domain is not authorized in Firebase. Please add the app URL to Authorized Domains in Firebase Console.');
      } else if (error.code === 'auth/credential-already-in-use') {
        toast.error('This account is already linked to another user.');
      } else if (error.code === 'auth/popup-blocked') {
        toast.error('Sign-in popup was blocked. Please allow popups for this site or open the app in a new tab to log in.', {
          duration: 6000,
        });
