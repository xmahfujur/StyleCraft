...
export default function ProductDetail() {
  ...
  return (
    <div className="pt-24 md:pt-32 pb-10 md:pb-24 max-w-7xl mx-auto px-4">
      <button onClick={() => navigate(-1)} className="flex items-center space-x-2 text-white/40 hover:text-gold transition mb-8 md:mb-12 group">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16">
        <div className="space-y-6">
          ...
          >
            <img 
              src={product.images?.[activeImage] || product.image || null} 
              alt={product.name} 
            ...
            <button
              ...
              className={`absolute top-4 right-4 md:top-6 md:right-6 p-3 md:p-4 rounded-full backdrop-blur-md transition-all duration-300 z-10 ${
            ...
            <div className="grid grid-cols-4 gap-2 md:gap-4 px-2 md:px-0">
              {product.images.map((img: string, idx: number) => (
                <div 
                  ...
                  className={`aspect-square bg-white/5 cursor-pointer border-2 transition flex items-center justify-center overflow-hidden rounded-sm ${activeImage === idx ? 'border-gold' : 'border-transparent hover:border-gold/30'}`}
                >
                  <img src={img || null} className="max-w-full max-h-full object-contain block" referrerPolicy="no-referrer" />
                </div>
              ))}
            </div>
          )}
        ...
        >
          <span className="text-gold tracking-[0.4em] text-[10px] md:text-xs mb-3 md:mb-4 uppercase">{product.category}</span>
          <h1 className="text-3xl md:text-6xl font-serif mb-4 md:mb-6 leading-tight">{product.name}</h1>
          {(() => {
            const variant = product.variants?.find((v: any) => v.size === selectedSize);
            ...
            return (
              <div className="flex items-center gap-4 mb-6 md:mb-8">
                {product.discountedPrice ? (
                  <>
                    <p className="text-2xl md:text-3xl font-bold text-gold">৳{product.discountedPrice}</p>
                    <p className="text-lg md:text-xl font-light text-white/30 line-through">৳{currentPrice}</p>
                  </>
                ) : (
                  <p className="text-xl md:text-2xl font-light text-white/80">৳{currentPrice}</p>
                )}
              </div>
            );
          })()}
